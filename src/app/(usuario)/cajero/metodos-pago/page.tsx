...
export default function PaymentMethods({ 
  totalToPay = 150, // Se asume que el padre pasa el total real
  onConfirmPayment, 
  onCancel 
}: Props) {
  ...
  // Obtiene el ID del pedido de la URL ?id=123
  ...
  const handleConfirm = async () => {
    ...
    if (!pedidoId) return alert("Error crítico: No se identificó el número de pedido en la URL.");
    if (totalToPay <= 0) return alert("Error: El monto a pagar es inválido.");
    ...
    // 2. Preparar Payload real para el Backend
    const payload = {
      pedidoId: pedidoId,
    ...
    try {
      setIsProcessing(true); // Bloquear botón

      // 3. Enviar al Endpoint Real
      ...
      // 4. Éxito
      // Callback opcional por si el padre necesita actualizar algo antes de irse
      onConfirmPayment?.(data);
      
      alert(`✅ Cobro registrado! Factura #${data.facturaId || 'Generada'}`);
      
      // Redireccionar al dashboard o vista de éxito
      ...
      setIsProcessing(false); // Liberar botón
    }
  };

  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-gray-50 dark:bg-black min-h-screen sm:min-h-0 sm:bg-transparent">
            ...
            <div className="mt-8 space-y-3">
              ...
              {onCancel && (
                <button 
                  onClick={onCancel}
                  disabled={isProcessing}
                  className="w-full bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 dark:text-gray-400 font-semibold py-3 rounded-xl transition-colors disabled:opacity-50"
                >
                  Cancelar
                </button>
              )}
